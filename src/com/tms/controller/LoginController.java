		...
		try {
			if (null == username || username.equalsIgnoreCase("undefined") || username.trim().length() == 0) {
				...
				loginRes.setErrorMsg(password + " - " + MyConstants.PASSWORD_BLANK);
					...
					|| securitycode.trim().length() == 0) && (null == loginVia || loginVia.trim().length() == 0 || loginVia.equalsIgnoreCase("web"))) {
				...
				if (session != null) {
					...
					} else if ((null == loginVia || loginVia.equalsIgnoreCase("web")) && (null == session.getAttribute("dns_security_code")
